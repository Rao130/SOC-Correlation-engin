class AdvancedAnalytics {
    ...
    updateAnalyticsStats() {
        ...
        if (trend) {
            ...
            trend.style.color = stats.trend === '↑' ? 'var(--status-critical)' : 
                              stats.trend === '↓' ? 'var(--status-low)' : 
                              'var(--status-medium)';
        }
    ...
    getTrendIcon(trend) {
        switch(trend) {
            case 'increasing': return { icon: '↑', color: 'var(--status-critical)' };
            case 'decreasing': return { icon: '↓', color: 'var(--status-low)' };
            default: return { icon: '→', color: 'var(--status-medium)' };
        }
    }
    ...
    calculateAnalyticsStats() {
        ...
        const avgResponseTime = total > 0 ? Math.round(Math.random() * 30 + 5) : 0;
        ...
        const trend = recentAlerts.length > 10 ? '↑' : recentAlerts.length > 5 ? '→' : '↓';
    ...
    createResponseTimesChart() {
        ...
        const responseTimeData = Array.from({length: 7}, () => Math.random() * 30 + 5);
